...
# ----------------- PAGE CONFIG -----------------
st.set_page_config(
    page_title="InfoBeans Foundation - Reporting Portal",
    page_icon="🎓",
    layout="wide",
...
st.markdown("""
    <style>
    .main-title { font-size: 26px; font-weight: 800; color: #0F172A; margin-bottom: 2px; }
    .sub-title { font-size: 13px; color: #64748B; margin-bottom: 20px; }
    .metric-box { background: white; padding: 14px; border-radius: 10px; border: 1px solid #E2E8F0; text-align: center; }
    </style>
...
def send_parent_email(student, report_month, sender_email, app_pwd):
    ...
    msg['To'] = target_email
    msg['Subject'] = f"Monthly Academic Progress Report - {student.get('StudentName')} ({student.get('RollNo')})"
...
# ----------------- SIDEBAR -----------------
with st.sidebar:
    st.markdown("### 🎓 InfoBeans Portal")
    st.caption("Multi-Batch Progress & Dispatch System")
    
    uploaded_file = st.file_uploader("Upload Multi-Batch Excel File", type=["xlsx", "xls"])
    
    st.markdown("---")
    st.markdown("#### ⚙️ Settings & Credentials")
    report_month = st.text_input("Report Month", value="September 2026")
    sender_email = st.text_input("Sender Gmail", value="pavanipandit64@gmail.com")
    app_password = st.text_input("16-Digit App Password", type="password", help="Generated from Google Account > Security > App Passwords")

# ----------------- MAIN LOGIC -----------------
st.markdown('<div class="main-title">🎓 Student Progress Reporting & Dispatch Portal</div>', unsafe_allow_html=True)
st.markdown('<div class="sub-title">Multi-batch tracking with 3rd/4th year segregation, WhatsApp messaging, and consolidated college reports.</div>', unsafe_allow_html=True)

if uploaded_file is None:
    st.info("👆 Please upload the Multi-Batch Excel File (`InfoBeans_Shuffled_Testing_MultiBatch.xlsx`) from the left sidebar to start.")
    st.stop()
...
try:
    ...
    st.error(f"Error reading Excel sheet: {e}")
    st.stop()
...
# ----------------- MASTER BULK DISPATCH -----------------
st.subheader("🚀 Master Batch Dispatch Actions")
act_c1, act_c2 = st.columns(2)

with act_c1:
    if st.button("📧 Send Individual Reports to All Filtered Parents", use_container_width=True, type="primary"):
        if not app_password:
            st.warning("⚠️ Please enter your 16-Digit Gmail App Password in the sidebar.")
        else:
            ...
            status_text.success(f"✅ Successfully dispatched {success_count} parent progress reports!")
...
with act_c2:
    if st.button("🏛️ Send Consolidated PDF + Excel to All Selected Colleges", use_container_width=True):
        if not app_password:
            ...
            status_text.success(f"✅ Successfully dispatched consolidated packets to {success_col_count} colleges!")

st.markdown("---")

# ----------------- SECTION 1: COLLEGE CARDS -----------------
st.subheader("🏛️ Institution / TPO Consolidated Packets")
    ...
    for idx, c_name in enumerate(col_cards):
        ...
        with grid_cols[idx % 2]:
            with st.container(border=True):
                ...
                st.caption(f"3rd Year: **{c_y3}** | 4th Year: **{c_y4}** | TPO: `{c_email}`")
...
st.subheader("👨‍🎓 Student Evaluation & Parent Dispatch Matrix")

for idx, s in enumerate(filtered_students):
    with st.container(border=True):
        row_c1, row_c2, row_c3, row_c4, row_c5 = st.columns([2.5, 2, 2.5, 1.5, 2.5])
        ...
        with row_c2:
            ...
            st.caption(f"Batch: {s['Batch']} | {s['Year']}")
        ...
        with row_c4:
            st.markdown(f"Overall: **{s['OverallScore']}%**")
        ...
        with row_c5:
            ...
            st.markdown(f'<a href="{wa_url}" target="_blank" style="display:inline-block;width:100%;text-align:center;background-color:#25D366;color:white;padding:6px;border-radius:6px;font-weight:600;font-size:12px;text-decoration:none;margin-bottom:4px;">💬 WhatsApp</a>', unsafe_allow_html=True)
            
            pdf_data = generate_student_pdf(s, report_month)
            st.download_button("📄 PDF", data=pdf_data, file_name=f"{s['RollNo']}_Report.pdf", mime="application/pdf", key=f"spdf_{idx}", use_container_width=True)
